...
export default function BiasAnalyzer() {
  ...
  const analyze = async (e: React.FormEvent) => {
    ...
    try {
      // First, find articles related to this topic
    ...
    <div>
      <Form onSubmit={analyze} className="d-flex gap-2 mb-4">
        <Form.Control
          ...
          disabled={loading}
        />
        <Button type="submit" variant="warning" disabled={loading || !topic.trim()}>
      ...
      {results.length === 0 && !loading && (
        <p className="text-secondary text-center py-4">
          Enter a topic to compare how different sources cover it.
        </p>
      )}
          ...
          <Card.Body>
            ...
            <Row className="g-2">
              {result.sources?.map((source, j) => (
                <Col key={j} md={6}>
                  <Card bg="black" text="light" className="border-secondary h-100">
                      ...
                      {source.propaganda_indicators?.length > 0 && (
                        <div className="d-flex flex-wrap gap-1 mt-1">
                          {source.propaganda_indicators.map((ind, k) => (
                            <Badge key={k} bg="dark" className="border border-secondary">
                              {ind}
                            </Badge>
                          ))}
                        </div>
